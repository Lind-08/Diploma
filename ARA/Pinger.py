import os
import l_lib.timers as timers
import logger
import db_module


def _ping(ip=""):
    res = os.popen("rm .ping_log; ping -c 1 -w 3 {0} >> .ping_log; echo $?".format(ip)).readlines()
    if res[0][0] == "0":
        return True
    else:
        return False
...
class Pinger:
    def __init__(self, db, ping_time=1000):
        self.state = False
        self.__db = db
        self.__ping_time = 1000
        self.__timer = timers.Timer(self.__ping_time, OnTimerFunc=self.ping_computers)
    ...
    def ping_computers(self):
        ...
        for x in self._managed_nodes:
            ...
            if not self._managed_nodes[x]:
                string = "Error pinging {0}".format(x)
                logger.write_log(string)
